...
#heatmap with Disease cluster only
df_clust1 <- as.data.frame(colData(rld1)[,"Disease"])
row.names(df_clust1) <- colnames(rld1)
pheatmap(new_mat1, annotation_col=df_clust1)
...
#SHRINKAGE
resAsh <- lfcShrink(dds2, coef="Disease.State_inflamed_vs_non.inflamed", type="normal")
resAsh

resAsh <- subset(resAsh, padj < 0.1)
resAsh <- resAsh[order(resAsh$log2FoldChange, decreasing=T), ]
head(resAsh)

#da evitare: LFC too high
res21 <- results(dds2,contrast = c("Disease.State", "inflamed", "non-inflamed") )
summary(res21)

res2_sten_noninf <- results(dds2,contrast = c("Disease.State", "stenotic", "non.inflamed") )
summary(res2_sten_noninf)
...
resSig21 <- subset(res21, padj < 0.1)
resSig21 <- resSig21[order(resSig21$log2FoldChange, decreasing=T), ]
head(resSig21)

resSig2_sten_noninf <- subset(res2_sten_noninf, padj < 0.1)
resSig2_sten_noninf <- resSig2_sten_noninf[order(resSig2_sten_noninf$log2FoldChange, decreasing=T), ]
head(resSig2_sten_noninf)

#too high LFC, so LFC shrinkage
